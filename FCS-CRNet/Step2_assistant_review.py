...
encoder_temp = keras.Model(inputs=[image_tensor], outputs=[encoder_network(image_tensor)])
decoder = keras.Model(inputs=[codewords_vector], outputs=[decoder_network(codewords_vector)])

adam = keras.optimizers.Adam(lr=0.0002)  # reset the learning rate
encoder.compile(optimizer=adam, loss='mse')
encoder_temp.compile(optimizer=adam, loss='mse')
...
file = 'CRNet_' + envir + '_dim' + str(encoded_dim) + 'Step2'
...
encoder.load_weights('result/CRNet_encoder_indoor_dim512_Step1_1_model.h5')
encoder_temp.load_weights('result/CRNet_encoder_outdoor_dim512_Step1_12_model.h5')
encoder.layers[18].set_weights(encoder_temp.layers[18].get_weights())

encoder.layers[18].trainable = False  # set the FC layer as untrainable

decoder.load_weights('result/CRNet_decoder_indoor_dim512_Step1_1_model.h5')
...
print("Step2_assistant_review has finished.")
